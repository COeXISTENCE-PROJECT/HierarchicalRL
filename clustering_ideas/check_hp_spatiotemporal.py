...
print("\nklastrowanie czasoprzestrzenne")
...
print(f">>> Co po fuzji (Przestrzeń + Czas) daje ostateczną liczbę klastrów: {best_final_k} <<<")
...
ax1.plot(best_initial_k, best_mse, marker='o', markersize=12, color='red', label=f'Wybrane początkowe K={best_initial_k}')
...
# Zaznaczenie wyniku na niebieskiej linii
ax2.plot(best_initial_k, best_final_k, marker='s', markersize=10, color='magenta')

ax2.set_ylabel('Ile klastrów zostało po fuzji?', color='blue', fontweight='bold')
ax2.tick_params(axis='y', labelcolor='blue')

plt.title(f'Spatiotemporal KMeans (Próg czasu: {TIME_THRESHOLD}s)')

# Łączenie legend z obu osi, żeby ładnie wyglądały
...
print("\nGotowe! Najlepsze parametry dla spatiotemporal:")
print(f"Początkowe num_clusters (K_initial): {int(best_initial_k)}")
print(f"Przewidywana Ostateczna liczba klastrów (po fuzji): {int(best_final_k)}")
