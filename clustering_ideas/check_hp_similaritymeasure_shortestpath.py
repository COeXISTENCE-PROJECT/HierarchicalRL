"""
FULL GRID SEARCH K-MEDOIDS PIPELINE
...
for current_k in k_list:
    for tw, sw in zip(time_weights_test, space_weights_test):
        print(f"[{current_run}/{total_runs}] Trenowanie: K={current_k} | TW={tw:.1f} | SW={sw:.1f}")
        ...
        for iteration in range(MAX_ITERS):
            ...
            for index, row in df.iterrows():
                distances = [calculate_network_distance(row, center, tw, sw) for center in centroids]
            ...
            for i in range(current_k):
                ...
                if not cluster_cars.empty:
                    ...
                    for _, car_row in cluster_cars.iterrows():
                        d = calculate_network_distance(car_row, virtual_center, tw, sw)
                        if d < min_dist:
                            min_dist = d
                            best_car = car_row.to_dict()
                    new_centroids.append({'t_norm': best_car['t_norm'], 'origin_real_id': best_car['origin_real_id'], 'dest_real_id': best_car['dest_real_id']})
                else:
...
ax1.set_ylabel('Błąd Przestrzenny (MSE)', color=color1, fontweight='bold')
...
ax2 = ax1.twinx()
color2 = 'tab:orange'
ax2.set_ylabel('Błąd Czasowy (MSE)', color=color2, fontweight='bold')
...
plt.title(f'Kompromis Wag dla K={best_k}')
fig.tight_layout()
plt.legend()
plt.savefig(PLOT_W_PATH)

print("\nNajlepsze parametry dla similarity measure (shortest path):")
