...
for current_k in k_list:
    for tw, sw in zip(time_weights_test, space_weights_test):
        print(f"[{current_run}/{total_runs}] Training with: K={current_k} | TW={tw:.1f} | SW={sw:.1f}")
...
# Find the "elbow point" on the list of best results for each K
...
plt.title(f'Trade-off for K={best_k}')
fig.tight_layout()
plt.legend()
plt.savefig(PLOT_W_PATH)

print("\nBest Parameters for Similarity Measure (Shortest Path):")
