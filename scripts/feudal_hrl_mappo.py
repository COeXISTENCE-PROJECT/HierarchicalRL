...
#                         manager_log_prob=prev["manager_log_prob"],
#                         controller_log_prob=prev["controller_log_prob"],
#                         extrinsic_reward=reward, # <- Sukces!
#                         intrinsic_reward=intrinsic_reward,
#                         manager_step=prev["manager_step"]
#                     )
#                     model.push(transition)
...
class ClusterMAPPOAgent(BaseLearningModel):
    def __init__(
        ...
        self.manager_reward_weight = float(config["manager_reward_weight"])
        self.manager_entropy_coef = float(config["manager_entropy_coef"])
        self.manager_epochs = int(config.get("manager_epochs", 3))
        self.controller_epochs = int(config.get("controller_epochs", 3))
    ...
    def act(self, local_state: np.ndarray, current_subgoal: int, manager_step: bool, physical_mask_np: np.ndarray) -> tuple:
        state_tensor = self._to_tensor(local_state)
        ...
        # Maska fizyczna x Maska celu (Fix 3)
        physical_mask = torch.as_tensor(physical_mask_np, dtype=torch.float32, device=self.device).unsqueeze(0)
        subgoal_mask = self._build_uniform_subgoal_mask(new_subgoal)
        action_mask = subgoal_mask * physical_mask
        
        # Zabezpieczenie: jeśli cel nie ma wspólnych fizycznych tras, użyj chociaż fizycznych
    ...
    def learn(self):
        ...
        advantages_np = np.zeros(len(batch), dtype=np.float32)
        
        # --- FIX 2: Grupowanie GAE po agent_id ---
        agent_indices = {}
        for i, b in enumerate(batch):
            agent_indices.setdefault(b.agent_id, []).append(i)
            
        for ag_id, indices in agent_indices.items():
            ag_rewards = [rewards[i] for i in indices]
            ag_values = [values_np[i] for i in indices]
            
            # FIX 7: Jeśli sekwencja agenta w tym batchu nie kończy się terminalem, używamy ostatniej wartości
            last_transition = batch[indices[-1]]
            next_val = 0.0 if last_transition.done else ag_values[-1]
                
            ag_adv = self.compute_gae(ag_rewards, ag_values, next_value=next_val).cpu().numpy()
            for adv_idx, batch_idx in enumerate(indices):
                advantages_np[batch_idx] = ag_adv[adv_idx]
        ...
        # --- FIX 4: Epoki PPO ---
        epochs = max(self.controller_epochs, self.manager_epochs)
        
        for epoch in range(epochs):
            # Odtworzenie masek do dystrybucji Controllera
            action_masks = []
            for b in batch:
                sg_mask = self._build_uniform_subgoal_mask(int(b.subgoal)).squeeze(0)
                p_mask = torch.as_tensor(b.physical_mask, dtype=torch.float32, device=self.device)
                comb = sg_mask * p_mask
                if comb.sum() == 0: comb = p_mask
                action_masks.append(comb)
            ...
            manager_indices = [i for i, b in enumerate(batch) if b.manager_step]
            if manager_indices:
                m_states = local_states[manager_indices]
                m_subgoals = subgoals[manager_indices]
                old_m_log_probs = torch.as_tensor([batch[i].manager_log_prob for i in manager_indices], dtype=torch.float32, device=self.device)
                m_rewards = torch.as_tensor([self.manager_reward_weight * batch[i].extrinsic_reward for i in manager_indices], dtype=torch.float32, device=self.device)
            ...
            # Świeża ocena krytyka (żeby nie uczyć się na starych outputach)
            fresh_values = self.critic(global_states).squeeze()
            if fresh_values.dim() == 0: fresh_values = fresh_values.unsqueeze(0)
            critic_loss = F.mse_loss(fresh_values, returns)
...
if __name__ == "__main__":
    ...
    for episode in range(training_eps):
        ...
        for agent_id in env.agent_iter():
            ...
            c_id = agent_to_cluster.get(agent_id, 0)
            model = cluster_models[c_id]
            terminal = termination or truncation
            ...
            # --- KROK 1: PUSH POPRZEDNIEGO DOŚWIADCZENIA ---
            if agent_id in agent_context:
                prev = agent_context[agent_id]
                
                # FIX 6: Brak darmowych punktów!
                intrinsic_reward = 0.0
                if prev.get("previous_subgoal") is not None and prev["subgoal"] != prev["previous_subgoal"]:
                    intrinsic_reward -= float(params.get("goal_switch_penalty", 0.0))
                ...
                transition = Transition(
                    ...
                    manager_log_prob=prev["manager_log_prob"],
                    controller_log_prob=prev["controller_log_prob"],
                    extrinsic_reward=float(reward), # To jest nagroda wypracowana przez starą akcję
                    intrinsic_reward=intrinsic_reward,
                    manager_step=prev["manager_step"],
                    agent_id=str(agent_id),
                    physical_mask=prev["physical_mask"],
                    done=terminal
                )
                model.push(transition)
                
                # Czyścimy tylko po pushu ostatniego kroku (FIX 1)
                if terminal:
                    del agent_context[agent_id]
            ...
            if terminal:
                ...
                # Pobierz fizyczną maskę z SUMO (FIX 3)
                physical_mask = info.get("action_mask")
                if physical_mask is None:
                    physical_mask = np.ones(action_size, dtype=np.float32)
                else:
                    physical_mask = np.array(physical_mask, dtype=np.float32)
                
                new_subgoal, manager_log_prob, action, controller_log_prob = model.act(
                    observation, current_sg, manager_step, physical_mask
                )
            ...
            # --- KROK 3: UCZENIE (ODBLOKOWANE) --- (FIX 5)
            if len(model.memory) >= model.batch_size:
                model.learn()

        # Logowanie po epizodzie
        log_data = {
            "episode": human_learning_episodes + episode,
            "training/reward_sum": float(np.sum(episode_rewards)),
            "training/reward_mean": float(np.mean(episode_rewards)),
    ...
    for episode in range(test_eps):
        ...
        for agent_id in env.agent_iter():
            ...
            if termination or truncation:
                ...
                new_subgoal, _, action, _ = model.act(observation, current_sg, manager_step, physical_mask)
